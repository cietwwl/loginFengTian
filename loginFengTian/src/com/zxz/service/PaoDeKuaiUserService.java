...
import com.zxz.dao.impl.PdkcardDaoImpl;
...
import com.zxz.util.PaoDekuaiWeiXinUtil;
...
public class PaoDeKuaiUserService implements Constants{

	private static Logger logger = Logger.getLogger(PaoDeKuaiUserService.class);  
	...
	private boolean loginWithCode(JSONObject jsonObject, ServletOutputStream out,String ip,HttpSession session) throws IOException {
		...
		try {
			JSONObject accessTokenJson = PaoDekuaiWeiXinUtil.getAccessTokenJson(code);
			String refreshToken = accessTokenJson.getString("refresh_token");
			String openid = accessTokenJson.getString("openid");
			String accesstoken = accessTokenJson.getString("access_token");
			JSONObject userInfoJson = PaoDekuaiWeiXinUtil.getUserInfo(accesstoken, openid);
			...
			record.setUserid(findUser.getId());
	 ...
	 * @return
	 */
	public boolean isRegistPdk(User user){
		PdkcardDao pdkCardDao =  new PdkcardDaoImpl();
		Pdkcard userPdkCard = pdkCardDao.selectByPrimaryKey(user.getId());
		if(userPdkCard==null){
			Pdkcard pdkcard = new Pdkcard();
			pdkcard.setUserid(user.getId());
			pdkcard.setRoomcard(DEFAULT_USER_REGIST_ROOMCARD_PDK);
			pdkcard.setRefreshToken(user.getRefreshtoken());
			pdkCardDao.insert(pdkcard);
			user.setRoomCard(DEFAULT_USER_REGIST_ROOMCARD_PDK);
			return true;
		}
		user.setRoomCard(userPdkCard.getRoomcard());
		return false;
	}
	...
	private boolean loginWithUnionid(JSONObject jsonObject,ServletOutputStream out,String ip,HttpSession session) throws IOException {
		...
		if(user!=null){//用户不存在
			Pdkcard pdkcard = pdkcardDao.selectByPrimaryKey(user.getId());
			if(pdkcard==null){
				notifyUserLoginFail(out,"errorRefreshToken","微信refreshToken过期请重新授权");
				return false;
			}
			String refreshToken = pdkcard.getRefreshToken();
			try {
				String accesstoekn = PaoDekuaiWeiXinUtil.getAccessTokenWithRefreshToken(refreshToken);
				JSONObject userInfo = PaoDekuaiWeiXinUtil.getUserInfo(accesstoekn, user.getOpenid());
				setUserWithUserInfoJson(userInfo, user);//封装用户的信息
				user.setRoomCard(pdkcard.getRoomcard());
	...
	private void notifyUserLoginSuccess(User findUser, ServletOutputStream out,boolean isFirstLogin,String ip) throws IOException {
		...
		Map<String, String> userInfoMap = RedisUtil.getHashMap("uid"+findUser.getId()+ "",REDIS_DB_PDK);
		String roomId = null;
		if(userInfoMap.size()!=0){
			roomId = userInfoMap.get("roomId");
		}
		if(!StringUtils.isNullOrEmpty(roomId)){
			//redis中得到用户的信息
			String roomInfo = RedisUtil.getKey("pkRoomId"+roomId,REDIS_DB_PDK);
		...
		RedisUtil.setHashMap("uid"+findUser.getId(),hashMap,REDIS_DB_PDK,EXPIRE_USER_LOGIN);
	}
...
//		findUser.setRefreshToken(refreshToken);
	...
	public JSONObject getUserSumScore(User user,JSONObject jsonObject) {
		...
		List<Map<String, Object>> userScoreList = userScoreDao.findUserSumScorePdk(map);
		JSONObject outJsonObject = new JSONObject();
...
//			object.put("total", baseScore.get("total"));
			...
			map2.put("roomid", baseScore.get("roomNumber"));
			map2.put("userid",  user.getId());
			int findUserScoreTotal = userScoreDao.findUserScoreTotalPdk(map2);
			object.put("total", findUserScoreTotal);
			object.put("total", findUserScoreTotal);
	...
	public void beforeCreateRoom(ServletOutputStream out,String path) throws Exception {
		...
		List<Server> serverList = XmlUtils.getServerList(path,paoDekuaiServer);
...
//				int totalLoginUser = jsonObject.getInt("totalLoginUser");
	...
	public String beforeEnterRoom(JSONObject serviceStr, String realPath) throws Exception {
		...
		String roomInfo = RedisUtil.getKey("pkRoomId"+roomId,3);
		if(StringUtil.isNull(roomInfo)){//房间不存在
			//如果redis中没有房间信息则遍历所有的游戏服务器获取数据
			List<Server> serverList = XmlUtils.getServerList(realPath,paoDekuaiServer);
			for(int i=0;i<serverList.size();i++){
...
//			out.write(result.getBytes("UTF-8"));
	...
	public JSONObject getUserScore(JSONObject serviceStr) {
		...
		List<UserScore> userScoreList = userScoreDao.findUserScorePdk(map);
		JSONObject outJsonObject = new JSONObject();
		JSONArray scoreArray = new JSONArray();
		for(int i=0;i<userScoreList.size();i++){
